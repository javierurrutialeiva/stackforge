...
class stack():
    ...
    def save(self):
        ...
        with h5py.File(f"profiles_z={z}.h5", "w") as f:
            ...
            for k in keys:
                f.create_dataset(k, np.array([h[k] for h in self.halos]))
...
class sampler():
    def __init__(self, x, data, priors, model, cov = None, 
                 models_dir = "models", priors_dir = "priors",
                 likelihood_kind = "gaussian"):
        self.data = data
        self.x = x
        if cov is None:
            self.cov = np.eye(len(data))
        elif cov is not None and np.ndim(cov) == 1:
            cov = np.diag(cov)
            
        profiles_module = importlib.import_module(models_dir)
        priors_module = importlib.import_module(priors_dir)
        
        model_func = getattr(profiles_module, model)
        ndims = len(priors)
        prior_funcs = []
        prior_args = []
        prior_names = []
        ...
        for i in range(ndims):
            p = priors[i]
            pname, pfunc, pargs = p.split("|")
            pargs = np.array(pargs.split(","), dtype = float)
            prior_funcs.append(getattr(priors_module, pfunc))
            prior_args.append(pargs)
            prior_names.append(pfunc)
    ...
    def run(self, nwalkers = 10, nsteps = 100, output_file = "./mcmc_samples.h5", 
            rewrite = True, ncores = 1):
        self.filename = output_file
        ndims = self.ndims
        likelihood_kind = self.likelihood_kind
        sampler_kwargs = {}
        sampler_kwargs["inv_sigma"] = np.linalg.inv(self.cov)
        sampler_kwargs["log_det_C"] = np.linalg.slogdet(self.cov)[1]
        sampler_kwargs["model"] = self.model
        if rewrite == True:
            if os.path.exists(output_file):
                os.remove(output_file)
        backend = emcee.backends.HDFBackend(output_file)
        if likelihood_kind == "chi2":
            def ln_likelihood(theta, x, y, sigma, **kwargs):
                inv_sigma = kwargs["inv_sigma"]
                model = sampler_kwargs["model"]
                y_model = model(x, theta)
                res = y - y_model
                chi2 = np.dot(res, np.dot(inv_sigma, res.T))
                return -0.5 * np.log(chi2), chi2, y_model
        ...
        def ln_prior(theta):
            prior = 0.0
            for i in range(len(prior_args)):
                args = prior_args[i]
                prior += prior_funcs[i](theta[i], *args)
            return prior
        def ln_posterior(theta, x, y, sigma, **kwargs):
            ln_lk, chi2, y_model = ln_likelihood(theta, x, y, sigma, **kwargs)
            ln_p = ln_prior(theta)
            posterior = ln_p + ln_lk
            if np.isnan(posterior) == True or np.isfinite(ln_lk) == False:
                posterior = -np.inf
            return posterior, chi2, y_model
        dtype = []
        dtype.append(("chi2", np.dtype((np.float64, 1))))
        dtype.append(("signal", np.dtype((np.float64, len(self.x)))))
        x = self.x
        y = self.data
        sigma = self.cov
        pool = Pool(ncores)
